...
from neuro_pilot.data.neuro_pilot_dataset import NeuroPilotDataset, custom_collate_fn
...
@dataclass
class TrainConfig:
    dataset_yaml: str = "/home/quynhthu/Downloads/data_final/data.yaml"
    batch_size: int = 16
    image_size: int = 320
    num_workers: int = 1
    epochs: int = 100
    ...
    lambda_heatmap: float = 3.0
    lambda_traj: float = 1.5
    lambda_det: float = 1.5
...
def build_criterion(cfg: TrainConfig, model, device: torch.device) -> CombinedLoss:
    ...
    base_config.loss.use_fdat = True
    base_config.loss.use_uncertainty = False
    ...
    torch.save(
        {
            ...
            "metrics": metrics,
            "model_cfg": "neuro_pilot/cfg/models/neuralPilot_dual.yaml",
            "names": names,
            "cfg": cfg,
...
def train(cfg: TrainConfig) -> None:
    ...
    data_cfg.data.augment.rotate_deg = 5.0
    data_cfg.data.augment.scale = 0.1
    data_cfg.data.augment.perspective = 0.05
    data_cfg.data.augment.hsv_h = 0.015
    data_cfg.data.augment.hsv_s = 0.4
    data_cfg.data.augment.hsv_v = 0.4
    data_cfg.data.augment.color_jitter = 0.1
    data_cfg.data.augment.fliplr = 0.0
    logger.info("Configured Light Augmentation (Rotate=5, Perspective=0.05, HSV=True, CJ=0.1) & Cosine LR")
