...
class LoadTensors:
    ...
    def __init__(self, source, imgsz=640):
        ...
        if source.ndim == 3:
            ...
            if isinstance(self.orig, np.ndarray):
                self.orig = [self.orig]
    ...
    def __next__(self):
        ...
        img0 = (
            self.orig[self.count]
            if isinstance(self.orig, (list, np.ndarray, torch.Tensor)) and len(self.orig) > self.count
            else None
        )
        self.count += 1
        ...
        if img.shape[-1] != self.imgsz or img.shape[-2] != self.imgsz:
            ...
            if img.ndim == 3:
                img = F.interpolate(
                    img.unsqueeze(0), size=(self.imgsz, self.imgsz), mode="bilinear", align_corners=False
                ).squeeze(0)
            else:
...
def get_dataloader(source, imgsz=640, vid_stride=1, auto=False):
    ...
    if isinstance(source, (str, Path)) and (
        str(source).startswith(("rtsp://", "rtmp://", "http://", "https://"))
        or (Path(source).exists() and Path(source).suffix == ".txt")
    ):
