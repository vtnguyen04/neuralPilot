...
class TimmBackbone(nn.Module):
    ...
    def __init__(self, model_name, pretrained=True, features_only=True, out_indices=None):
        ...
        try:
            self.model = timm.create_model(
                model_name, pretrained=pretrained, features_only=features_only, out_indices=out_indices
            )
            ...
            if pretrained:
                ...
                try:
                    self.model = timm.create_model(
                        model_name, pretrained=False, features_only=features_only, out_indices=out_indices
                    )
            ...
            if base_name != model_name:
                ...
                try:
                    self.model = timm.create_model(
                        base_name, pretrained=pretrained, features_only=features_only, out_indices=out_indices
                    )
                    return
                except:
                    if pretrained:
                        try:
                            self.model = timm.create_model(
                                base_name, pretrained=False, features_only=features_only, out_indices=out_indices
                            )
                            return
                        except:
                            pass
    ...
    def get_channels(model_name):
        ...
        try:
            ...
            if "mobilenetv4_conv_small" in model_name:
                return [32, 32, 64, 96, 960]
            if "mobilenetv4_conv_medium" in model_name:
                return [32, 48, 80, 160, 960]
            if "mobilenetv4_conv_large" in model_name:
                return [24, 48, 96, 192, 960]
...
class NeuroPilotBackbone(nn.Module):
    ...
    def __init__(self, backbone_name="mobilenetv4_conv_medium", num_commands=4, dropout_prob=0.0):
        ...
        self.output_channels = {
            "p3": 64 if "small" in backbone_name else 128,
